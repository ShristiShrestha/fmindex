class Fmindex:
    ...
    def match_alt(self, pattern, wavelet_tree, bwt_len):
        s = 0 # 0
        e = bwt_len
        for i in range(len(pattern) - 1, -1, -1):
            if pattern[i] in self.dict.keys():
                ...
                # acc to blog, here rank query expects count value
                # up to (including) positions s and e
                s = self.dict[pattern[i]] + wavelet_tree.rank_query_alt(pattern[i], s)
                e = self.dict[pattern[i]] + wavelet_tree.rank_query_alt(pattern[i], e)
                if s > e:
                    return 0
            else:
                return 0
